...
public class SkillController : NetworkBehaviour
{
    ...
    // スキルの準備状態
    public void StanbySkill(int CharNum, bool _freeMode)
    {
        if (skillFlag)
        {
            skillStanby = true;
    ...
    // スキル使用
    public void UseSkill(int CharNum, bool _freeMode)
    {
        if (skillStanby)
        {
            skillStanby = false;
